...
## [0.0.7] - 2026-04-02
### Added
- 新增文件右键菜单“打开文件”，支持二级菜单选择“VS Code 中打开”与“系统浏览器打开”，并同时覆盖编辑器右键菜单与资源管理器右键菜单。

## [0.0.6] - 2026-04-02
### Changed
- 优化 Vue Hover 触发逻辑：在 `.vue` 模板中仅对组件标签名显示组件提示，避免将 `type`、事件名或普通属性误识别为组件。

## [0.0.5] - 2025-11-28
### Changed
- props 扫描与监听逻辑现仅处理 `src/**/*.vue`，相关文档同步更新，避免误扫 JS 文件。
- 生成流程会把未声明 props 的组件写入 JSON，并在 Hover 中以“该组件未声明 props”提示，确保依然能看到组件来源。
- 没有 props 的情况下也会输出组件基础信息并给出友好通知，生成日志/提示语更加清晰。
